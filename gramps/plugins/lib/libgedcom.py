...
class GedcomParser(UpdateCallback):
    ...
    def __init__(self, dbase, ifile, filename, user, stage_one,
        ...
        self.name_parse_tbl = {
            ...
            TOKEN__RUFNAME  : self.__name__rufname,    # daubnet Ages
        ...
        self.adopt_parse_tbl = {
            ...
            TOKEN_CHAN   : self.__ignore,
            TOKEN_QUAY   : self.__ignore,
            # Not legal, but inserted by daubnet Ages
            TOKEN__ASSO  : self.__event__asso,
        }
        ...
        # the actual link was ignored. Title matching was used.
        #
        # n  @XREF:_LOC@ _LOC      {1:1}
        #    +1 NAME <PLACENAME>   {1:1} p.*
        #    +1 POST <POSTAL_CODE> {0:1} p.*
        #    +1 MAP  <MAP>         {0:1} p.*
        #       +2 LONG...
        #       +2 LAT...
        #    +1 NOTE @<XREF:NOTE>@ {0:1} p.*

        self._loc_func = {
            TOKEN_NAME   : self.___loc_name,
            TOKEN_POST   : self.___loc_post,
            TOKEN_MAP    : self.__place_map,
            TOKEN_NOTE   : self.___loc_note,
            TOKEN_RNOTE  : self.___loc_note,
            }
        self.func_list.append(self._loc_func)
        ...
        self.event_place_map = {
            ...
            TOKEN__LOC   : self.__event_place_loc,
            TOKEN_MAP    : self.__place_map,
    ...
    def __find_flat_place(self, title, location):
        """
        Finds an existing place based on the title not checking the primary location.
        ...
        for place_handle in self.place_names[title]:
            ...
            if place.get_title() == title:
                ...
                return_value = place
        if i>1:
            self.__add_msg(_("the location name "+title+" is not unique! Additional information from the _loc table might not have been added to the right place."), None, None)
        # return the most recently found place
        return return_value
    ...
    def __add_place(self, event, sub_state):
        ...
        if sub_state.place:
            ...
            if place is None:
                # if place is not found, then search for the flattened name
                # e.g. place "A" has place "B" in its reflist.
                # title is "A", 
                place = self.__find_flat_place(sub_state.place.get_title(),
                                      self.__get_first_loc(sub_state.place))
            if place is None:
    ...
    def __parse_record(self):
        ...
        while True:
            ...
            elif key in ("_LOC", "LOCATION"):
                self.__parse_loc(line)
    ...
    def __name__rufname(self, line, state):
        """
        @param line: The current line in GedLine format
        @type line: GedLine
        @param state: The current state
...
#            name = self.__parse_name_personal(line.data)
    ...
    def __event_place_loc(self, line, state):
        """
        @param line: The current line in GedLine format
        @type line: GedLine
        @param state: The current state
    ...
    def __event__asso(self, line, state):
        """
        @param line: The current line in GedLine format
        @type line: GedLine
        @param state: The current state
        ...
        pers = self.__find_or_create_person(self.pid_map[line.data])
        event_ref = EventRef()
        ...
        while True:
            ...
            if self.__level_is_finished(line, state.level+1):
                ...
                if line.data in ("GODPARENT"):
                    role = EventRoleType(
                        EventRoleType.GODFATHER)
                else:
                    role = EventRoleType(
                        (EventRoleType.CUSTOM, line.data))
                event_ref.set_role(role)
        pers.add_event_ref(event_ref)
        self.dbase.commit_person(pers, self.trans)
    ...
    def __obje_file(self, line, state):
        ...
        # this is not specified, but daubnet Ages exports the top level nodes into this subnode
...
# FTM non-standard DATE in OBJE, treat as Media Date.
    ...
    def __parse_loc(self, line):
        ...
        self.__parse_level(state, self._loc_func, self.__undefined)

        if state.place:
            self.dbase.commit_place(state.place, self.trans)

    def ___loc_name(self, line, state):
        title = line.data
        place = self.__find_flat_place(title, None)
        if place is None:
            place = Place()
            place.set_name(PlaceName(value=title))
            place.set_title(title)
    ...
    def ___loc_post(self, line, state):
        state.place.set_code(line.data)

    def ___loc_note(self, line, state):
    ...
    def __header_sour(self, line, state):
        ...
        if line.data.strip().lower() in ['rootsmagic', 'ages']:
